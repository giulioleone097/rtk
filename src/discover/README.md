...
**Compound splitting** — The rewrite engine walks the tokens, splitting on `Operator` (`&&`, `||`, `;`) and typed `Pipe` tokens (`|`, `|&`). For normal pipelines, producers and intermediate stages stay raw, and only an argument-safe final stage marked `pipeline_final_safe` is rewritten. The initial safe set is ordinary `grep` and `rg` invocations; search pattern-file forms (`-f`/`--file`) defer because they can consume pipeline stdin as configuration. Stderr pipelines (`|&`) and pipelines containing opaque shell groups remain raw. The one exception to the raw producer is a pipeline whose every stage after the producer only displays what it reads — `head` or `cat` with flags and counts alone: nothing there parses the stream, so the producer is rewritten exactly as it would be on its own and the consumer stages are copied verbatim. `tail`, `less` and `more` stay out, whatever the producer, because every RTK view is capped or regrouped: its tail is not the tail of the raw output, and a pager cannot reach past the cap. Every filter caps its own diagnostics, so `cargo test 2>&1 | tail -50` after a rewrite loses the failure names past the cap that the raw tail prints, and `git log | tail -5` would show the end of the 10 newest commits instead of the oldest ones.

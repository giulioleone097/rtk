...
**Compound splitting** — The rewrite engine walks the tokens, splitting on `Operator` (`&&`, `||`, `;`) and typed `Pipe` tokens (`|`, `|&`). For normal pipelines, producers and intermediate stages stay raw, and only an argument-safe final stage marked `pipeline_final_safe` is rewritten. The initial safe set is ordinary `grep` and `rg` invocations; search pattern-file forms (`-f`/`--file`) defer because they can consume pipeline stdin as configuration. Stderr pipelines (`|&`) and pipelines containing opaque shell groups remain raw. The one exception to the raw producer is a pipeline whose every stage after the producer only displays what it reads — `head` or `cat` with flags and counts alone: nothing there parses the stream, so the producer is rewritten exactly as it would be on its own and the consumer stages are copied verbatim. `tail` joins that set only when the producer is one of `SUMMARY_PRODUCERS` (cargo test/build/clippy/check, jest, vitest, playwright test, pytest, go test, bun test, with or without `npx`), whose filter summarises the whole run: the last lines of that summary answer what `| tail -50` asked. After anything else `tail` stays out, together with `less` and `more`, because an RTK view of a list is capped and regrouped: its tail is not the tail of the raw output (`git log | tail -5`) and a pager cannot reach past the cap. `pnpm test`, `npm test`, `yarn test` and `dotnet test` have no filter at all, so their pipelines stay raw.

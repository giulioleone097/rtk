...
//!
//! Not yet wired into the tool router: the caller that reaches
//! [`fetch_and_index`] is added once this lands on `tokenaut/main`, so
//! `dead_code` is allowed here until then instead of loosening the crate's
//! `warnings = "deny"` lint.
#![allow(dead_code)]
